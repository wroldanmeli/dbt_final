    ...
    create or replace table `metrics-streams-dev`.`billing_wr_test`.`final_tmp_BillingV2`
...
-- depends_on: `metrics-streams-dev`.`TemporalData`.`final_tmp_BillingV1`
...
WITH get_current_day AS (
    SELECT CASE WHEN CAST('3' AS INT64) >= 20 THEN 
...
SELECT A.* FROM `metrics-streams-dev`.`TemporalData`.`final_tmp_BillingV1` A , finance_limits B
